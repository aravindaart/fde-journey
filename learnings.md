...
- 2026-06-24: Git history correction — script24 was committed twice with conflicting scope labels (`feat(week4): add script24 eval harness` and `feat(week4): add script24 eval harness` under what should have been a week5 scope). The underlying work is not duplicated, only the commit message scope drifted. Left pushed history as-is rather than rewriting a public repo's history for a cosmetic label issue. Lesson: check `git log --oneline -5` before committing when crossing a week boundary mid-script.
- 2026-07-04: LangGraph models agent control flow as a graph of nodes and edges instead of a while loop. The value over a raw loop is explicit branching, named state, and built-in checkpointing - not new capability, just structure that scales better when flow gets complex.
- 2026-07-04: A node does work, an edge decides what runs next, and state is the shared data passed between nodes.
...
- 2026-07-08: Compound queries can make Claude emit multiple `tool_use` blocks in a single assistant turn. The original `execute_tools` used `next()` to grab only the first tool_use block, silently dropping any others — the dropped tool_use id then had no matching tool_result, causing Anthropic API to reject the next request with a 400 (`tool_use ids were found without tool_result blocks`). Fix: collect ALL tool_use blocks in the message via list comprehension, loop over them, and append one tool_result block per tool_use into a SINGLE user message — not separate messages, since Anthropic requires the result immediately after in the next message, and splitting into multiple messages would leave later tool_use ids still unsatisfied in the intermediate messages.
- 2026-07-08: The response extraction result["messages"][-1]["content"][0]["text"] assumes the first content block is always a text block. This is fragile because future responses may contain different content block types or multiple blocks; a more robust extraction should iterate over the content blocks and select the appropriate text block(s).
- 2026-07-08: Compound multi-part queries can produce inconsistent final answers even when all tool_results are correctly captured in state — in one run Claude's final text included all three tool results, in another it silently dropped one despite the tool_result being present in messages. This is a generation-level reliability gap, not a state/loop bug: the fix is prompt-level (explicit instruction to address every part of a multi-part question), not code. Deferred — out of scope for Week 6 (Docker + deploy); relevant to flagship system prompt design in Week 7+.
- 2026-07-10: Converting a LangGraph CLI application to FastAPI does not require changing the graph itself. The only changes are replacing input() with a Pydantic request model, moving graph.invoke(initial_state) into a route handler, and returning a Pydantic response model. The LangGraph execution flow, nodes, edges, and state remain identical because graph.invoke() is independent of whether it is called from __main__ or an HTTP endpoint. The initial state, however, must still satisfy the full AgentState contract (including fields like iteration and stop_reason), otherwise downstream routing logic that reads those keys will fail with KeyError.
- 2026-07-10: A successful Docker build does not guarantee a deployable web service. The image originally still used CMD ["python", "script27_langgraph.py"], which built successfully but launched an interactive CLI instead of an HTTP server, so the container never bound to the port expected by Render. For web deployments, the runtime entry point must start Uvicorn (or another ASGI server) and listen on the runtime PORT. Also, Docker's exec-form CMD (["..."]) does not perform shell variable expansion, so using "$PORT" there passes the literal string to the process; either use a shell (sh -c) or configure Render's Start Command to perform the $PORT substitution
- 2026-07-10: Deploying the LangGraph agent to Render highlighted that the free web service uses an ephemeral filesystem. The bundled bubble_tea.txt is available because it is part of the Docker image, but the local chroma_db/ directory is not persisted across deployments or instance restarts. On startup the application recreates the Chroma collection by re-ingesting the source data, which is acceptable for a small demo but would become increasingly expensive as the knowledge base grows. For a production system, the vector store should be persisted externally (e.g. a managed vector database or persistent disk) rather than rebuilt on every cold start.